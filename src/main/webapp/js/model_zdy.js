...
(function($){
	...
	var uploadClass = {
		...
		startUpload : function(options) {
			...
			if(uploadfile && (uploadfile.size >= options.maxSize || options.maxSize == 0)){
		...
		addUploadFile:function(data,fileType,fileListContainer,mode){
			if(fileType == "img"){
				...
				if(mode == "form"){
					$li.append($("<span>",{"class":"file-size mr15"}).text("("+this.renderSize(data.len || data.size)+")")).append($("<a>",{"href":"javascript:;"}).text("删除").on("click",function(){
						$li.remove();
	...
	var model = {
		...
		createForm : function($container,options,purpose){
			...
			switch(formType){
				...
				case "advSelect":
					...
					this.createDiv(options,purpose).append($input).appendTo($container);
					var $input2 = $("<input>",{"type":"input","data-name":options.field,"name":options.mappingField || (options.field+"-advSelect"),"class":"form-control prevent advSelect"})
					$input.before($input2);
					var list;
							...
							setting.onCheck = function(personCheckedArr, $simpleInput){
								...
								if($.isArray(personCheckedArr) && personCheckedArr.length > 0) {
									$.each(personCheckedArr, function(index, obj) {
										if(obj[options.valueConfig.conditionKey]){
				...
				case "file":
					...
					uploadClass.uploadInit($div,{
						url:options.valueConfig.url,
						type:"file",
						maxCount: options.maxSize != null ? parseInt(options.maxSize) : 0, //最大数量
						maxSize: options.maxValue != null ? parseInt(options.maxValue) : 0 //上传文件最大大小
					})
					return false;
				case "img":
					var $div = this.createDiv(options,purpose).appendTo($container);
		...
		assignment:function(viewData,$container,modelData,selector){
			...
			$.each(modelData, function(index,obj) {
				...
				viewData[obj["field"]] = viewData[obj["mappingField"]] || viewData[obj["field"]] || "";
				if((obj["formType"] === "file" || obj["formType"] === "img") && viewData[obj["field"]] && viewData[obj["field"]].length > 0){
					...
					if($.isArray(attach)){
						getAttach(attach)
					}else if(obj["valueConfig"] && obj["valueConfig"]["url"]){
			...
			$container.find(selector).each(function(){
				...
				switch($this.attr("formtype")){
					...
					case "advSelect":
						if(typeof(value) == "string"){
							...
							var valueKey = newModelData[$(this).attr("name")]["valueConfig"]["valueKey"] || "value",valueList = [];
							$.each(value, function(index,obj) {
								valueList.push(obj[valueKey])
							});
							$(this).val(valueList.toString());
						}
				...
				}else if($(this).hasClass("advSelect") && value){
					if(typeof(value) == "string"){
						$(this).val(value)
					}else if($.isArray(value)){
