...
(function () {
    ...
    operationWidth = 200,//menu操作按钮的总宽度
    leftOperationWidth = 40,//左侧按钮的宽度
    ...
    move = function (selDom) {
        ...
        var releft = selDom.offset().left - $(".index-sidebar").width();
    ...
    init = function () {
         ...
         * 初始化菜单控件
         */
        $("#menu-list").on("click","a.main",function(){
        	var jthis = $(this);
	...
	window.tabOpen = function(linkUrl,linkHtml){
        ...
        if (selDom.length === 0) {
            ...
            //创建menu,绑定点击事件
            var iel = $("<span>", { "class": "glyphicon glyphicon-remove" }).on("click", closemenu);
            $("<a>", {
                "html": linkHtml,
                "data-url": linkUrl,
                "data-value": linkHtml
	...
	window.tabClose = function(){
		
		$("#menu-list a.active").animate({ "width": "0", "padding": "0" }, 0, function () {
	...
	window.tabCloseAll = function(){
		/*$("#menu-list a:not(.main) span").each(function(){
			$(this).trigger("click")
		})*/
		$("#page-content iframe:not(.main)").remove();
		$("#menu-list a:not(.main)").remove();
		$("#menu-list a.main:eq(0)").trigger("click");
		$("#menu-list").css({marginLeft:"0px"});
	}
	window.tabCloseOther = function(){
		$("#page-content iframe:not(.main):not(.active)").remove();
		$("#menu-list a:not(.main):not(.active)").remove();
		$("#menu-list").css({marginLeft:"0px"})
	}
	
    $.fn.tab = function (options) {
    	this.off("click");
    	$("#menu-list").off("click","a.main");
    	$("#page-prev,#page-next").off("click");
    	if(options != "destroy"){
	        init();
	        //绑定点击事件
	        this.on("click", function () {
	            ...
	            //链接文字
	            var linkHtml = $.trim(this.text);
				tabOpen(linkUrl,linkHtml)
	            return false;
	        });
    	}
...
$(function(){
	$("#changePass").on("click",function(){
		var $modal = $("#changeMyPassModal").modal("show")
		$modal.resetForm();
		$modal.find("button.save").off("click").on("click",function(){
			if(!$.ver($modal.find(".form-control[required=required],.form-control[valueType]"))){
				return false;
			}
			var queryData = $modal.sendForm();
			if(queryData.newPasword == queryData.newPasword2){
				$.ajax({
					url:"yh/updatePassword.do",
					data:queryData,
					suc:function(data){
	...
	$.ajax({
		url:"/app-admin/Login/loginCheck.do",
		suc:function(data){
			if(data.yh){
				$("#user-info").html(data.yh.xm || "");
				window.userInfo = data.yh || null;
				window.localStorage.setItem("userInfo",data.yh ? JSON.stringify(data.yh) : null)
			}
		}
	})
	...
	$.ajax({
		type:"get",
		url:"gn/findGNList.do",
		suc:function(data){
			var $menu = $('.sidebar-menu');
			$menu.find(".treeview").remove();
			$.each(data.pList, function(index,obj) {
				if(obj.pId == "-1"){
					$menu.append($("<li>",{class:"treeview"}).append($("<a>",{href:"#"}).html('<i class="fa fa-files-o"></i> <span>'+obj.mc+'</span> <i class="fa fa-angle-right pull-right"></i>')).append($("<ul>",{"class":"treeview-menu","data-id":obj.gnId})))
				}else{
					var $ul = $menu.find("ul.treeview-menu[data-id="+obj.pId+"]");
					$ul.append($("<li>").html('<a href="'+(obj.url || "#")+'"><i class="fa fa-circle-o"></i> '+obj.mc+'</a>'))
				}
			});
			$.sidebarMenu($menu);
			$menu.find(".treeview-menu li a").tab();
		},
		hasAlert:true,
		loadingContainer:$('.sidebar-menu')
	});
	
	$(document).on("click","a.close-all-tabs",tabCloseAll).on("click",".close-other-tabs",tabCloseOther)
})
