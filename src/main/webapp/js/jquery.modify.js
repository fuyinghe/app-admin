(function(window, undefined) {
	...
	jQuery.extend({
		...
		ajax: function(url, options) {
			...
			if(s.loadingContainer && jQuery.fn.loading) {
				s.loadingContainer.loading()
			}
			...
			function done(status, nativeStatusText, responses, headers) {
				...
				if(isSuccess) {
					...
					if(success.errcode == "0" && s.suc) {
						s.suc(success, statusText, jqXHR)
					} else{
						if(s.hasAlert) {
							jQuery.errCodeRoute(success.errcode, success.errmsg, s.onErrConfirm)
						}
						if(s.err && typeof s.err == "function") {
							statusText = 'failed';
							s.err(jqXHR, statusText, success)
						}
					}
				} else {
					deferred.rejectWith(callbackContext, [jqXHR, statusText, error]);
					if(s.hasAlert) {
						jQuery.errStatusRoute(statusText, error, s.onErrConfirm)
					}
					if(s.err && typeof s.err == "function") {
						s.err(jqXHR, statusText, error)
					}
				}
				...
				if(s.loadingContainer && jQuery.fn.endLoading) {
					s.loadingContainer.endLoading()
				}
			}
			return jqXHR
		},
