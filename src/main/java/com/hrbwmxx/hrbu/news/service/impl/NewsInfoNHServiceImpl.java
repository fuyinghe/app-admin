...
@Service
public class NewsInfoNHServiceImpl implements INewsInfoNHService {
	// 新闻
	@Autowired
	private NewsInfoNHMapper newsInfoMapper;
	// 图片
	@Autowired
	private NHFileUploadService nhFileService;
	// 异常
	@Autowired
	...
	public Result queryNewsInfoListPage(Page page, NewsInfoNHCustom obj) {
		...
		List<NewsInfoNHCustom> newsCustomListAll = new ArrayList<NewsInfoNHCustom>();
		try {
	...
	public Result queryNewsInfoById(NewsInfoNHCustom obj) {
		NewsInfoNHVO re = new NewsInfoNHVO();
		try {
			if (obj.getId() != null && !"".equals(obj.getId())) {
				NewsInfoNHCustom info = newsInfoMapper.queryNewsInfoById(obj);
				if (info != null) {
					// 查看之后增加阅读次数
					String readTimes = String.valueOf(Integer.parseInt(info.getReadTimes()) + 1);
					NewsInfoNHCustom news = new NewsInfoNHCustom();
					...
					if (info.getPic() != null) {
						map_request.put("attachId", info.getPic());
						List<Map<String, String>> list_file_pic = nhFileService.queryAttrList(map_request);
						info.setList_file_pic(list_file_pic);
					}
				} else {
					re.setErrcode("500");
	...
	public Result saveNewsData(NewsInfoNHCustom obj, HttpServletRequest request) {
		...
		try {
			if(obj != null) {
				...
				//如果添加了图片
				if (obj.getPic()!=null){
					nhFileService.updateFileStateByIds(obj.getPic(), "1");
				}
				//保存内容
	...
	public Result updateNewsInfo(NewsInfoNHCustom obj, HttpServletRequest request) {
		...
		try {
			if(obj != null) {
				...
				//查一下原来的内容
				NewsInfoNHCustom info = new NewsInfoNHCustom();
	...
	public Result deleteNewsInfo(NewsInfoNHCustom obj) {
		...
		try {
			if(obj.getId()!=null){
				NewsInfoNHCustom temp = new NewsInfoNHCustom();  
		   ...
		   if (!"".equals(ids)){
			  ...
			  //得到所有的附件列表
	             ...
	             for(int i=0;i<attrs.length;i++) {
	            	 ...
	            	 nhFileService.deleteStateInvalidByAttachId(attrs[i]);
	            	 //清除文件
