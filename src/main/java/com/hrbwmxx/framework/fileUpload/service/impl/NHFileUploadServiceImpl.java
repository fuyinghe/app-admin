...
import com.hrbwmxx.framework.fileUpload.dao.NHFileUploadMapper;
...
@Service
public class NHFileUploadServiceImpl implements NHFileUploadService{
	public static Logger logger = LoggerFactory.getLogger(NHFileUploadServiceImpl.class);
	@Autowired
	private NHFileUploadMapper fileMapper;
	...
	private Map<String, String>   serverTransferToDb(MultipartFile file,Map<String, String>  map_request) throws IOException {
		...
		String baseHttpPath=properties.getProperty("baseNHPath");
		...
		map_request.put("downloadUrl", properties.getProperty("serverNHPath")+PropertiesUtil.SEPARATOR+map_request.get("code")+PropertiesUtil.SEPARATOR+map_request.get("name"));
		map_request.put("state",Constant.SYS_STATE_0+"");//是否启用Y/N
		map_request.put("attachId",attachId);//是否启用Y/N
	
		fileMapper.saveFileInfoData(map_request);
	...
	private Map<String, String> transferToDirServer(MultipartFile file, Map<String, String> map_request) throws  Exception {
		...
		rootDir=properties.getProperty("serverNHUrl");
	...
	* @return Map<String,String>
	...
	private Map<String, String> transferToDir(MultipartFile file, Map<String, String> map_request) throws  Exception {
		...
		//1.1获得文件根目录配置
		properties=PropertiesUtil.getKey("system.properties");
		rootDir=properties.getProperty("baseNHDir");
		oriName= file.getOriginalFilename();
		//1.2获得文件分割符
		separator =PropertiesUtil.SEPARATOR;
		//1.3创建相对路径
	...
	* @return  List<Map<String, String>>
	...
	public List<Map<String, String>> queryAttrList(Map<String, String> map_request) {
		...
		return fileMapper.queryAttrList(map_request,attrs);
	}
	/**
	 * 查询无效信息
	 */
	...
	public List<Map<String, String>> queryStateInvalidList() {
		...
		return fileMapper.queryStateInvalidList();
	}
	 /**
	...
	@Override
		public Result updateFileStateByIds(String ids,String state){
			...
			try{
				...
				fileMapper.updateFileStateByIds(attrs, state);
	  ...
	  */
	  ...
	  @Override
		public Result deleteStateInvalidByAttachId(String attachId){
			RowsVo vo = new RowsVo();
			try{
				fileMapper.deleteStateInvalidByAttachId(attachId);
